...
%% Aunque en el sentido original de Spencer "apto" efectivamente podría referirse más a la sobrevivencia, en la síntesis evolutiva
%% moderna la sobrevivencia se vincula con el fitness solamente en tanto reproducción. A la sobrevivencia por sí misma no se le
%% otorga ningún papel en los modelos teóricos, hasta donde sé.
...
%% Esto es redundante, podrías decir más bien que "Esta definición caracteriza..."
...
%% La definición no es diferencial, lo que es diferencial es la reproducción. En dado caso podrías decir "Esta definición en
%% términos de la reproducción diferencial..."
...
%% Los 2 párrafos anteriores hay que revisarlo con cuidado. Si bien la frase de Spencer sí se interpretó como tautológica, esto fue al
%% entender "apto" en el sentido de la sobrevivencia. Eso fue parte de lo que llevó a la reformulación del fitness en términos de
%% la reproducción o de la combinación de sobrevivencia y reproducción. Sí se ha señalado el carácter tautológico de la selección
%% natural, por ejemplo por Popper o más recientemente por algunos autores de los que buscan entender la selección en un sentido
%% causal y no solamente estadístico, pero para mostrar esto hay que "hilar más fino". Te recomiendo un texto de Fox-Keller y
%% Lloyd (1992) donde puedes revisar un poco de las historia y de las diferentes caracterizaciones del fitness:
%% @book{keller_keywords_1992,
...
%% 	year = {1992},
%% 	keywords = {Evolución (Biología), Terminología}
%% }
...
Hay opciones disponibles para definir el concepto de \emph{fitness} que pueden salvar de la carga tautológica que parecen tener
las explicaciones por selección natural. 
%% Hasta aquí sólo has señalado el posible carácter tautológico del concepto de fitness
%% (aunque no estoy muy seguro de la argumentación) y no lo has argumentado explícitamente en relación con la selección natural y
%% menos con respecto a las explicaciones por selección natural. No te digo que no haya problemas con los modelos que reconstruyen
%% la SN, pero necesitas mostrar los puntos problemáticos.
...
%% Yo te sugeriría abandonar la expresión de Spencer en la medida de lo posible, a menos que pienses que tu propuesta puede
%% elucidarla en un sentido no tautológico.
Por tanto, necesitamos una mejor estrategia que nos ayude a definir la aptitud que tienen unos organismos y no otros.
%% Un objetivo mejor sería el ofrecer una caracterización del fitness que permita distinguir la selección natural de la deriva
%% génica, esto es, que muestre cómo la reproducción diferencial no siempre se trata de un fenómeno estocástico.
...
Para poder salvar al \emph{fitness} de la carga tautológica
%% Más que con respecto al concepto de fitness, la acusación de tautológica se ha dirigido hacia la selección natural. Sería
%% conveniente que enfatizaras esa relación para que se entienda la importancia de caracterizar el fitness de la manera adecuada.
...
%% La última oración hay que revisarla.
...
su progenie.
%% Como insertaste la cita de Godfrey-Smith, se pierde la referencia inmediata a las características ventajosas, pues (como
%% notarás) él está entendiendo fitness ya en términos exclusivamente reproductivos.
...
Pero esto no es suficiente para incorporar causalidad a las explicaciones evolutivas. No lo es porque es claro que las
...
%% No se entiende a qué se refiere "el hecho de que esta característica apoye" ¿Qué es lo que quieres decir?
...
%% Endler distingue, en el Cap. 1 de su libro, la selección propiamente dicha y la respuesta genética como dos componentes de un
%% proceso de SN.
...
%%  Sería bueno que incluyeras algunas de las referencias de quienes hablan de la Síntesis Extendida
...
Regresemos a alguno de nuestros ejemplos anteriormente señalados. La luz y la temperatura afectan el proceso de florecimiento en
...
%% Por la redacción anterior no queda claro si la temperatura y la luz afectan independientemente la floración, o si las
%% variaciones en la floracións se deben a la relación entre ambos factores. Por lo que explicas adelante se ve que es lo primero,
%% pero esto debería quedar claro desde el principio.
...
\begin{center}
  \begin{tikzpicture}[scale=2]
    \node (X) at (1,1){Florecimiento}; %% El término más común en biología es floración
...
Pensemos en un ejemplo más. El experimento realizado por Amarillo Suárez y Fox \citeyear{Amarillo-Suarez2006}. Hay insectos que se
...
%% En la última afirmación al menos a mi no me queda claro en qué sentido *observar* el tamaño de los padres y una *hipótesis*
%% contarían como intervenciones alternativas desde la perspectiva de Woodward

\begin{center}
  \begin{tikzpicture}[scale=2]
    \node (X) at (1,1){Tamaño de los organismos};
    \node (A) at (0,0){Tamaño de los padres};
    \node (B) at (2,0){Semillas del hospedero};%% ¿Esta intervención se realizó a partir del tamaño de las semillas o más bien a
    %% partir del efecto de que los organismos se desarrollen en uno u otro hospedero? Esto es, ¿las autores descartaron la
    %% posibilidad de que las diferencias en el desarrollo de S. limbatus pudieran deberse no al tamaño de las semillas del
    %% hospedero, sino a algún otro factor?
    \path[-angle 90,font=\scriptsize]
    (A) edge    (X)
    (B) edge    (X);
    \end{tikzpicture}
...
%% Más que un organismo, en la caracterización de Godfrey-Smith de la SN se habla de variantes y en general se entiende el fitness
%% como algo que opera sobre clases de organismos.
...
%% Falta explicitar cuál sería la definición causal de fitness que propones
...
%% Hay que ver cómo exactamente lo expresan Walsh et al., pero más que selección debida a la deriva génica lo correcto sería evolución o cambio en las frequencias alélicas debido a deriva o a selección natural. Si ellos se refieren a ambos procesos como selección, entonces es muy probable que estén incurriendo en petición de principio
...
Hay algunas razones para rechazar la segunda premisa: que deriva génica y selección natural son indistinguibles. En primer lugar, la selección natural no es indiscriminada. Esto significa que cuando hay selección natural en una población, esperamos que algunos de los organismos sean más aptos que otros en un medio ambiente.
%% En un sentido causal esta relación sería al revés: si hay diferencias en cuanto a la aptitud en un cierto ambiente sería de esperarse que hubiera selección natural, en caso contrario, y particularmente si la población es pequeña, se esperaría que hubiera deriva génica.
...
%% Al nivel de la genética de poblaciones, tanto la deriva génica como la selección natural se reconocen como factores que alteran el equilibrio de Hardy-Weinberg, la diferencia entre deriva y selección tiene que ver con si el consecuente cambio en las frequencias alélicas se explica como producto del azar (un error de muestreo) o como resultado de diferencias de aptitud (o fitness). En el primer caso hablaríamos de deriva y en el segundo de selección natural.
...
En la sección anterior dijimos por qué es ventajoso aceptar una interpretación causal de \emph{fitness}. Entre las razones esgrimidas estuvo que es un concepto de fitness que es explicativo, pero además nos permite distinguir entre deriva génica y selección natural. Bouchard y Rosenberg \cite{Bouchard2004} nos dicen que la definición tradicional de \emph{fitness} tiene tres problemas. El primer problema está relacionado con las frecuencias: si \emph{fitness} tuviera una interpretación puramente estadística, entonces decimos que ``a la larga'' el organismo más apto dejará más descendencia. Sin embargo, nos interesa que el tiempo sea finito, porque de otra manera no tendríamos acceso a dichas frecuencias. El segundo problema que tiene una definición de este tipo es que es una tautología. Se supone que los organismos más aptos, esto es, con más \emph{fitness} son aquellos que dejan más descendencia. Pero si definimos el grado de \emph{fitnes} de un organismo como aquél que deja más descendencia, entonces los organismos que dejan más descendencia son aquellos organismos que dejan más descendencia. Para resolver este problema, nosotros apelamos a una definición causal de \emph{fitness}. La solución de Bouchard y Rosenberg está en línea con esta propuesta. Para resolver el problema de la tautologicidad, %% Este término no sé si existe, mejor habría que expresarlo de otra manera.
...
Hay al menos dos aspectos problemáticos en la definición anterior. La comparación dos a dos en una población puede volverse complicada tomando en cuenta el tamaño de la población. Para poblaciones grandes este proceso podría tardar un tiempo indefinido. %% Este problema quizás podría solventarse con técnicas de muestreo como las que se emplean comúnmente en el estudio de las poblaciones biológicas.
...
%% En general se ha definido, en el ámbito de la genética de poblaciones, a la evolución (no a la evolución por selección natural) como el cambio en las frecuencias alélicas.
...
Pero entiendo que este \textbf{sólo}, %% Más bien "este señalamiento" o algo así.
